...
public class PapagoService implements IPapagoService {
    ...
    public PapagoDTO translate(PapagoDTO pDTO) throws Exception {
        ...
        String psotParms = "source=" + source + "&target=" + target + "&text=" + URLEncoder.encode(text, "UTF-8");

        log.info("postParms : " + psotParms);

        String json = NetworkUtil.post(IPapagoService.translateApiURL, this.setNaverInfo(), psotParms);
